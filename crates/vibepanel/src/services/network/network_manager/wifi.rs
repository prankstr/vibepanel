...
impl NmService {
        ...
        W: std::future::Future<Output = Result<(), String>>,
    {
        ...
        if outcome.is_err() || cancel.is_cancelled() {
            // Deactivate only this attempt. NM removes profiles that remain volatile.
            if let Err(e) = call(
                super::NM_PATH,
                NM_IFACE,
...
pub(super) async fn wait_activation(
    ...
    let result = glib::future_with_timeout(timeout, async {
        loop {
            ...
            // but never let a state overwrite a queued terminal error (cancel or bus closure).
            while event.is_ok()
...
mod tests {
    ...
    fn unavailable_service_cancels_both_activation_attempts() {
        ...
        for available in [true, false] {
            ...
            let mobile = gio::Cancellable::new();
            service.wifi.activation_cancel.replace(Some(wifi.clone()));
            service
                .mobile
                .activation_cancel
                .replace(Some(mobile.clone()));
            service.set_unavailable();
            assert!(wifi.is_cancelled() && mobile.is_cancelled());
        }
    }
    ...
    fn activation_signal_runner() {
        ...
        context.with_thread_default(|| context.block_on(async {
            ...
            for case in ["already_active", "signal", "fast_signal", "failure_before_read", "failure", "cancel", "cancel_then_state", "timeout"] {
    ...
    /// deleting after an error. Profile proxies target the captured unique owner.
    async fn profiles_handle_partial_failures(
        address: &str,
        ...
        original
            .call_future(
                ...
                "org.freedesktop.DBus",
                "RequestName",
                Some(&(NM_SERVICE, 0u32).to_variant()),
                None,
                gio::DBusCallFlags::NONE,
                5000,
            )
    ...
    fn wifi_profile_lifecycle() {
        ...
        for (existing, activated, cancel, cancel_during_save, save_fails, password) in [
            (true, true, false, false, false, None),
            (true, true, false, false, false, Some("ignored password")),
            (true, false, false, false, false, Some("ignored password")),
            (true, true, true, false, false, None),
            (false, true, false, false, false, Some("password")),
            (false, false, false, false, false, Some("password")),
            (false, true, true, false, false, Some("password")),
            (false, true, false, false, true, Some("password")),
            (false, true, false, true, false, Some("password")),
            (false, true, false, true, true, Some("password")),
        ] {
                ...
                |path, _, method, args| {
                    std::future::ready((|| {
                        ...
                        match method {
                            ...
                            "DeactivateConnection" => {
                                assert!(!activated || cancel || cancel_during_save);
                                assert_eq!(
            ...
            assert_eq!(result.is_ok(), activated && !cancel && !cancel_during_save);
            assert_eq!(watched.get(), !cancel);
            if save_fails && !cancel_during_save {
            ...
            assert_eq!(
                ...
                !activated || cancel || cancel_during_save
            );
            if cancel_during_save {
                let update = calls.iter().position(|call| call == "Update2").unwrap();
                let deactivate = calls
                    .iter()
                    .position(|call| call == "DeactivateConnection")
                    .unwrap();
